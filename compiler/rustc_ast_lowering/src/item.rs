                ...
                match (&contract.requires, &contract.ensures) {
                    (Some(req), Some(ens)) => {
                        ...
                        let (req_span, precond) = {
                            ...
                            (req_span, this.stmt_expr(req.span, precond))
                        };

                        let ens_span = this.lower_span(ens.span);
                        let postcond_checker = {
                            let ens_span = this.mark_span_with_reason(
                                DesugaringKind::Contract,
                                ens_span,
                                None,
                            );
                            let lowered_ens = this.lower_expr_mut(&ens);
                            this.expr_call_lang_item_fn(
                                ens_span,
                        ...
                        let (check_hir_id, postcond_decl) = {
                            ...
                            let contract_check =
                                this.arena.alloc(this.expr(req_span, contract_check));
                            (
                                check_hir_id,
                                this.stmt_let_pat(
                                    None,
                        ...
                        let body =
                            this.inject_ensures_check(body, ens_span, check_ident, check_hir_id);

                        // Flatten the body into precond, then postcond, then wrapped body.
                        let wrapped_body = this.block_all(
                            body.span,
                            this.arena.alloc_from_iter([postcond_decl].into_iter()),
                            Some(body),
                        );
                    ...
                    (None, Some(ens)) => {
                        ...
                        let contract_check = {
                            ...
                            let then_block_stmts =
                                this.block_all(ens_span, stmts, Some(postcond_checker));
                            let then_block = this.arena.alloc(this.expr_block(&then_block_stmts));
                        ...
                        let (check_hir_id, postcond_decl) = {
                            ...
                            let contract_check =
                                this.arena.alloc(this.expr(ens_span, contract_check));
                            (
                                check_hir_id,
                                this.stmt_let_pat(
                                    None,
                    ...
                    (Some(req), None) => {
                        ...
                        let precond_check = {
                            ...
                            let then_block_stmts = this.block_all(req_span, stmts, None);
                            let then_block = this.arena.alloc(this.expr_block(&then_block_stmts));
                        ...
                        let precond_check = this.expr(req_span, precond_check);
                        let precond_check = this.stmt_expr(req_span, precond_check);
                        let body = this.arena.alloc(body(this));

                        // Flatten the body into precond, then wrapped body.
                        let wrapped_body = this.block_all(
                            body.span,
                            this.arena.alloc_from_iter([precond_check].into_iter()),
                            Some(body),
                        );
                        (params, this.expr_block(wrapped_body))
                    }
                    (None, None) => (params, body(this)),
                }
            } else {
                (params, body(this))
            }
